...
if [ -n "${EXVARS:+1}" ] ; then  # check if env var is already set
  ...
  EXVARS="bwat,bwatvel,diffusivity,temppabase,tempicethk_basal,bmelt,tillwat,velsurf_mag,mask,thk,topg,usurf,taud_mag,flux_divergence,velsurf,climatic_mass_balance,climatic_mass_balance_original,ocean_kill_flux,ocean_kill_flux"
  if [ "$5" = "hybrid" ]; then
