...
cmd="PISM_DO="" PARAM_SIAE=3 PARAM_PPQ=0.50 PARAM_CALVING=float_kill  REGRIDFILE=$REGRIDFILE PISM_OFORMAT=$OFORMAT STARTEND=$START,$END PISM_DATANAME=$PISM_DATANAME $BEDDEF PISM_BCFILE=$PISM_BCFILE ./run.sh $NN $CLIMATE $DURA $GRID hybrid null $OUTFILE $INFILE"
