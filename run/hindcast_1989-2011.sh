...
for K in 1e15 1e16 1e17; do
    for THK in 100 200 300; do
        ...
        PISM_OCEAN_BCFILE=ocean_forcing_${GRID}m_1989-2011_sb.nc
        FSOFT=0.8
        EXPERIMENT=${CLIMATE}_${TYPE}_${STARTYEAR}_${ENDYEAR}_k_${K}_calving_${CALVING}_${THK}_sbfrac_${FSOFT}
        SCRIPT=hindcast_g${GRID}m_${EXPERIMENT}.sh
        POST=hindcast_g${GRID}m_${EXPERIMENT}_post.sh
        rm -f $SCRIPT $POST
        ...
        cmd="PISM_DO="" PISM_CONFIG=$CONFIG PARAM_NOAGE=foo PARAM_FRACTURE=foo PARAM_FRACTURE=foo PARAM_FSOFT=$FSOFT PARAM_CALVING=$CALVING PARAM_CALVING_THK=$THK REGRIDFILE=$REGRIDFILE PISM_SURFACE_BCFILE=$PISM_SURFACE_BCFILE PISM_OCEAN_BCFILE=$PISM_OCEAN_BCFILE PISM_TIMEFILE=$PISM_TIMEFILE PISM_OFORMAT=$OFORMAT PISM_DATANAME=$PISM_DATANAME TSSTEP=daily EXSTEP=$EXSTEP PISM_SAVE=$SAVESTEP REGRIDVARS=litho_temp,enthalpy,tillwat,bmelt,Href,thk PARAM_SIAE=$E PARAM_PPQ=$PPQ PARAM_TEFO=$TEFO PARAM_TTPHI=$PARAM_TTPHI PARAM_SSA_N=$SSA_N ./run.sh $NN $CLIMATE 30 $GRID hybrid $HYDRO $OUTFILE $INFILE"
        echo "$cmd 2>&1 | tee job.\${PBS_JOBID}" >> $SCRIPT
