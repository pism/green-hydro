...
SCRIPTNAME=${CLIMATE}-spinup-${TYPE}_eis.sh
...
SCRIPT="do_${GRID}km_${CLIMATE}-spinup-${TYPE}_eis.sh"
rm -f $SCRIPT
EXPERIMENT="${DURAKA}ka ${CLIMATE}-climate initialization $TYPE"
MKA=$(($END/-1000))

OUTFILE=g${GRID}km_m${MKA}ka_${CLIMATE}_${TYPE}_eis.nc
...
cmd="PISM_DO="" STARTEND=$START,$END PISM_DATANAME=$PISM_DATANAME  ./run-eismint.sh $NN $CLIMATE $DURA $GRID hybrid null $OUTFILE $INFILE"
echo "$cmd 2>&1 | tee job.\${PBS_JOBID}" >> $SCRIPT

echo "($SPAWNSCRIPT)  $SCRIPT written"

GRID=10
SCRIPTNAME=${GRID}km_${CLIMATE}-spinup-${TYPE}_eis.sh
INFILE=pism_Greenland_${GRID}km_v2_${TYPE}.nc
REGRIDFILE=$OUTFILE
PISM_DATANAME=$INFILE
DURA=20000
...
echo >> $SCRIPT
cmd="PISM_DO="" STARTEND=$START,$END PISM_DATANAME=$PISM_DATANAME REGRIDFILE=$REGRIDFILE ./run-eismint.sh $NN $CLIMATE $DURA $GRID hybrid null $OUTFILE $INFILE"
echo "$cmd 2>&1 | tee job.\${PBS_JOBID}" >> $SCRIPT
echo "($SPAWNSCRIPT)  $SCRIPT written"
...
OUTFILE=g${GRID}km_0_${CLIMATE}_${TYPE}_eis.nc
...
echo >> $SCRIPT
cmd="PISM_DO="" STARTEND=$START,$END PISM_DATANAME=$PISM_DATANAME REGRIDFILE=$REGRIDFILE PARAM_FTT="foo" ./run-eismint.sh $NN $CLIMATE $DURA $GRID hybrid null $OUTFILE $INFILE"
echo "$cmd 2>&1 | tee job.\${PBS_JOBID}" >> $SCRIPT
echo "($SPAWNSCRIPT)  $SCRIPT written"
